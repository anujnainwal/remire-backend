...
// Add unique indexes to ensure one GIC account request per user and prevent duplicate email/phone
createGicAccountSchema.index({ user: 1 }, { unique: true });
createGicAccountSchema.index({ email: 1 }, { unique: true });
createGicAccountSchema.index({ mobileNumber: 1, countryCode: 1 }, { unique: true });
